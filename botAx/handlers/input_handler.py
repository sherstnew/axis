...
async def data_confirm(message: Message, state: FSMContext):
    ...
    if len(text) != 8 or not check_digits(text) or [i for i in text if float(i) < 0] or not 1 <= int(text[6]) <= 10:
        await message.answer(
    ...
    await message.answer(text=f'Результатыℹ️:\n1)Для общественного транспорта:\n  Утреннеий поток:\n      '
                              ...
                              f'        - заполненость станций - {round(response[0][0][0][1], 5)}%\n'
                              f'        - увеличение заполненость станций - {round(response[0][0][0][2], 5)}%\n'
                              f'    Вечерний поток:\n      '
                              f'- увеличение пассажиропотока на {round(response[0][1][0][0], 5)}\n'
                              f'        - заполненость станций - {round(response[0][1][0][1], 5)}%\n'
                              f'        - увеличение заполненость станций - {round(response[0][1][0][2], 5)}%\n'
                              f'2)Для дорог:\n'
                              f'    - увеличение транспортных средств на {round(response[1][0][0], 5)} т.с.\n'
...
async def choose_data_confirm(message: Message, state: FSMContext):
    ...
    if (len(text) != 10 or not check_digits(text) or [i for i in text if float(i) < 0]
            or not -90 <= float(text[0]) <= 90 or not -180 <= float(text[1]) <= 180 or not 1 <= int(text[6]) <= 10):
        await message.answer(
            text=f"Данные введены неверно🚫!\nПопробуйте еще раз в точности по списку:\n{useful_data_1}")
        return
